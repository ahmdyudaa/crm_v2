...
abstract class REST_Controller extends CI_Controller {
    ...
    protected $_message_type = [
            "ok" => "",
            "timeout" => "Expired Services",
            "notfound" => "Id not Registered",
            "invalid" => "Invalid Signature",
            "uncompleted" => "Header Variable Uncompleted"
        ];
    ...
    protected function mj_authenticate(){
        $api_id = $this->config->item('rest_id');
        $api_signature = $this->config->item('rest_signature');
        $api_timestamp = $this->config->item('rest_timestamp');
        $status = TRUE;
        
        $this->rest->message = "OK";
        $this->rest->key = NULL;
        $this->rest->level = NULL;
        $this->rest->user_id = NULL;
        $this->rest->ignore_limits = FALSE;
        ...
        // Find the key from server or arguments
        if (isset($this->_args[$api_id]) & isset($this->_args[$api_signature]) & isset($this->_args[$api_timestamp])){
            //$this->_args[$api_id];
            //$this->_args[$api_signature];
            //$this->_args[$api_timestamp];
            

            if($this->_args[$api_timestamp] < strtotime("-30 seconds")) {
                // Expired Services
                $status = FALSE;
                $this->rest->message = "timeout";
            }else{
                $ids = $this->db->where('id', $this->_args[$api_id])->get('ids')->row();
                if (sizeof($ids) > 0) {
                    $data = $this->_args[$api_id]."&".$this->_args[$api_timestamp];
                    $signature = hash_hmac('sha256', $data, $ids->keys, true);
                    $encodedSignature = base64_encode($signature);
                    //die($encodedSignature ." ". $this->_args[$api_signature]);
                    if($encodedSignature === $this->_args[$api_signature]){
                        $status = TRUE;
                    }else{
                        // Invalid Signature
                        $status = FALSE;
                        $this->rest->message = "invalid";
                    }
                }else{
                    // id not found
                    $status = FALSE;
                    $this->rest->message = "notfound";
                }
                
            }
        }else{
            $status = FALSE;
            $this->rest->message = "uncompleted";
        }
